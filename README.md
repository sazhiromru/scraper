...
Данные собираются поочередно с трех сайтов и сохраняются в формате CSV. 
  
Для примера выкладываю скрипт сбора данных с сайта с бесконечной прокруткой. 
...
1. Стандартные настройки free tier  подходят почти везде, в RDS отключаем auto-scaling, в EC2 - расширенный мониторинг, связываем RDS c EC2 при создании для удобства
...
2. Создаем необходимые SQL запросы для отображения основных показателей. Пример SQL запроса, используещего оконные функции для расчета посуточного KPI приложен ниже. Показатель расчитывает эффективность сделки посуточно из расчета покупки предметов на 500 долларов
